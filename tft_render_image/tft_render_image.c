 ...
 * With Frame Buffer for Simple Image Rendering
 */
...
/* ========== Frame Buffer ========== */
// Allocate in SRAM: 320x240x2 bytes = ~150KB (fits in 256KB SRAM)
...
/* ========== Frame Buffer Functions (Simple Rendering) ========== */

// Fill entire frame buffer with a color
...
// Draw a rectangle in frame buffer (simple, no clipping for brevity)
void FB_DrawRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color) {
    for (uint16_t row = y; row < (y + h); row++) {
        for (uint16_t col = x; col < (x + w); col++) {
            if (row < SCREEN_HEIGHT && col < SCREEN_WIDTH) {
                frame_buffer[(row * SCREEN_WIDTH) + col] = color;
            }
        }
    }
}

// Push frame buffer to TFT (bulk transfer)
void TFT_PushFrameBuffer(void) {
    TFT_SetAddressWindow(0, 0, SCREEN_WIDTH - 1, SCREEN_HEIGHT - 1);
    
    // Treat buffer as uint8_t array for bulk write
    uint8_t *buf_bytes = (uint8_t *)frame_buffer;
    uint32_t buf_len = (SCREEN_WIDTH * SCREEN_HEIGHT * 2);  // 16-bit = 2 bytes per pixel
    
    TFT_WriteDataBulk(buf_bytes, buf_len);
}

/* ========== Text Drawing Functions (Direct to TFT) ========== */
// (Kept as-is for tests; for full FB integration, could render text to FB too)
...
int main(void) {
    ...
    // Clear screen to black (direct)
    TFT_FillScreen(COLOR_BLACK);
    
    // Display header (direct)
    ...
    // Simple frame buffer "image" rendering (for understanding)
    // Render to buffer: red background with blue rectangle
    FB_Fill(COLOR_RED);
    FB_DrawRect(50, 50, 100, 100, COLOR_BLUE);
    
    // Push frame buffer to TFT (bottom half for demo)
    // Note: To render full screen, adjust window; here partial for visibility with text
    TFT_SetAddressWindow(0, 180, SCREEN_WIDTH - 1, SCREEN_HEIGHT - 1);
    uint8_t *buf_bytes = (uint8_t *)frame_buffer;
    uint32_t partial_len = ((SCREEN_WIDTH * 60) * 2);  // Bottom 60 rows only
    TFT_WriteDataBulk(buf_bytes + (SCREEN_WIDTH * 180 * 2), partial_len);  // Offset to bottom
    
    // Display "Hello world" message (direct)
